...
| [`fig_term_expansion_across_languages_gpt`](experiments/term_expansion/by_language_pair/gpt/figures/fig_term_expansion_across_languages_gpt.pdf) | `experiments/term_expansion/by_language_pair/gpt/figures/` | [`term_expansion/by_language_pair/gpt`](experiments/term_expansion/by_language_pair/gpt/README.md) | `python shared/lib/analysis/generate_result_figures.py --only by_language_pair_gpt` | `shared/results/dev_v1/{original,expand,cleaned}/` |
| [`fig_term_expansion_qwen_size_stacked`](experiments/term_expansion/by_language_pair/qwen/figures/fig_term_expansion_qwen_size_stacked.pdf) | `experiments/term_expansion/by_language_pair/qwen/figures/` | [`term_expansion/by_language_pair/qwen`](experiments/term_expansion/by_language_pair/qwen/README.md) | `python shared/lib/analysis/generate_result_figures.py --only by_language_pair_qwen` | `shared/results/dev_v1/{original,expand,cleaned}/` |
...
Generate all at once with `python shared/lib/analysis/generate_result_figures.py` (writes each into its home dir above; `dataset_comparison` writes 3 files, one per term mode). Of these, `fig_term_expansion_across_models`, `fig_term_expansion_across_languages_gpt`, `fig_lora_epoch_ablation`, and `fig_lora_best_models` are copied to `poster/figures/` for the poster. Every experiment's figures are copied to `report/figures/` for the paper, under a matching `report/figures/<experiment>/<sub-experiment>/` directory.
...
| ------ | -------- | ------- |
| `compare_by_model_and_language.py` (`--mode {all,proper_term}`) | [`experiments/term_expansion/by_language_pair/shared/scripts/`](experiments/term_expansion/by_language_pair/README.md) | Compares GPT, Qwen 3B, and Qwen 7B, and `ende`/`enru`/`enes` — writes both axes' report tables in one run. |
...
| `figure_by_language_pair_gpt.py` | [`experiments/term_expansion/by_language_pair/gpt/scripts/`](experiments/term_expansion/by_language_pair/gpt/README.md) | GPT-4o-mini only, grouped by language pair. |
| `figure_qwen_size_comparison.py` | [`experiments/term_expansion/by_language_pair/qwen/scripts/`](experiments/term_expansion/by_language_pair/qwen/README.md) | Qwen2.5-3B vs. 7B, stacked bars, grouped by language pair. |
