...
| [`fig_dev_v1_vs_dev_v2_training`](experiments/term_expansion/dataset_comparison/figures/fig_dev_v1_vs_dev_v2_training.pdf) | `experiments/term_expansion/dataset_comparison/figures/` | [`term_expansion/dataset_comparison`](experiments/term_expansion/dataset_comparison/README.md) | `python src/analysis/generate_result_figures.py --only dataset_comparison` | `results/dev_v1/original/`, `results/dev_v2/` |
...
`experiments/` groups six experiments into three named directories. [`term_expansion/`](experiments/term_expansion/README.md) holds four analyses over the shared `data/` → `results/` → `report/` pipeline documented below — each owns a `scripts/` folder with its table/figure-generating code (see their READMEs for exact reproduce commands) and `dataset_comparison`/`gpt_proposed_terms` each also own one small experiment-local `data/` folder for input that only they consume. `baseline/` and `lora_finetuning/` are self-contained model-run experiments with their own `data/`/`results/`.
...
| [`term_expansion/dataset_comparison/`](experiments/term_expansion/dataset_comparison/README.md) | `dev_v1` (test) vs. `dev_v2` (training) set comparison, GPT baseline. |
| [`term_expansion/gpt_proposed_terms/`](experiments/term_expansion/gpt_proposed_terms/README.md) | GPT-4o-mini `proper_term` (oracle dictionary) vs. `gpt_proposed_term` (GPT self-extracts/proposes terminology), zero-shot on `dev_v1/original`. |
| [`lora_finetuning/`](experiments/lora_finetuning/README.md) | LoRA fine-tuning of Qwen2.5 (3B/7B) vs. GPT-4o-mini and Qwen base, with an Excel export pipeline. |
...
Two dictionary-building scripts are experiment-specific and live in [`experiments/term_expansion/dataset_comparison/scripts/`](experiments/term_expansion/dataset_comparison/README.md):
...
| `compare_datasets_to_excel.py`, `compare_v1_variants_to_excel.py`, `figure_dataset_comparison.py` | [`experiments/term_expansion/dataset_comparison/scripts/`](experiments/term_expansion/dataset_comparison/README.md) | Compares `dev_v1/original` vs `dev_v2`, and `dev_v1/original` vs `dev_v1/dictionary`. |
...
| ---------- | ----------- | --------------- |
| [`experiments/term_expansion/by_model/report/`](experiments/term_expansion/by_model/README.md#report-tables) | `scripts/compare_models_to_excel.py` | `model_comparison.xlsx` (one sheet per dataset variant: `dev_v1_original_zero_shot`, `dev_v1_original_few_shot`, `dev_v1_expand`, `dev_v1_cleaned`, `dev_v2`) |
...
| [`experiments/term_expansion/dataset_comparison/report/`](experiments/term_expansion/dataset_comparison/README.md#report-tables) | `scripts/compare_datasets_to_excel.py`, `scripts/compare_v1_variants_to_excel.py` | `dev_v1_original_vs_dev_v2_dataset_comparison.xlsx` (one sheet per model), `dev_v1_original_vs_dev_v1_dictionary_gpt_comparison.xlsx` |
| [`experiments/term_expansion/gpt_proposed_terms/report/`](experiments/term_expansion/gpt_proposed_terms/README.md) | `scripts/compare_gpt_pipeline_modes_to_excel.py` | `dev_v1_original_gpt_pipeline_mode_comparison.xlsx` |
...
All data lives in `data/`, grouped by dataset (`dev_v1/`, `dev_v2/`) rather than by processing stage. Two exceptions live inside the experiment that's their only consumer instead: `dev_v1_gpt_proposed/`, under [`experiments/term_expansion/gpt_proposed_terms/data/`](experiments/term_expansion/gpt_proposed_terms/README.md), and `dev_v2_dictionary/` (the term dictionary built from dev_v2), under [`experiments/term_expansion/dataset_comparison/data/`](experiments/term_expansion/dataset_comparison/README.md).
...
| `dev_v1/dev_v1_dictionary/` | dev_v1 enriched from the term dictionary (output of `apply_dictionary_to_dev_v1.py`, [`experiments/term_expansion/dataset_comparison/scripts/`](experiments/term_expansion/dataset_comparison/README.md)). |
