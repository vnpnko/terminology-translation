...
| File | Regenerate |
| ---- | ---------- |
| `report/dev_v1_original_model_comparison.xlsx` | `python experiments/01_term_expansion_by_model/scripts/compare_models_to_excel.py results/dev_v1/original/with-few-shots` (or `.../no-few-shots`; `results/dev_v1/original` has no `gpt`/`qwen_3b`/`qwen_7b` directly — see `report/README.md` §3.4.1) |
...
`proper_term`-only comparison across dev_v1 term-list variants (`original`/`expand`/`cleaned`), rows grouped by variant then language, generated by [`scripts/compare_proper_term_across_models_to_excel.py`](scripts/compare_proper_term_across_models_to_excel.py) (sibling to `02_term_expansion_by_language_pair/report/dev_v1_proper_term_across_languages.xlsx`, same color scheme):

| File | Regenerate |
| ---- | ---------- |
| `report/dev_v1_proper_term_across_models.xlsx` | `python experiments/01_term_expansion_by_model/scripts/compare_proper_term_across_models_to_excel.py` |
