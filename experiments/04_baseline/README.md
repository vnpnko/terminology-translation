...
Both notebooks auto-detect input data from `data/raw/dev_v1_original/`, `data/interim/dev_v1_expand/`, `data/interim/dev_v1_cleaned/` (repo root), `experiments/04_baseline/`, or the notebook's working directory — set `DATA_DIR` explicitly if detection fails.

- `notebooks/openai_baseline.ipynb` requires `OPENROUTER_API_KEY` (see root [`.env.example`](../../.env.example)). Equivalent CLI: [`src/run_openai_baseline.py`](../../src/run_openai_baseline.py).
