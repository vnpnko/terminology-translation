...
Writes one .xlsx file (default: ``experiments/04_lora_finetuning/report/epoch_ablation.xlsx``)
...
Usage::
    ...
    python experiments/04_lora_finetuning/scripts/compare_epochs_to_excel.py
"""
...
def write_sheet(ws: Worksheet, rows_by_lang: dict[str, dict[int, list[float | None]]]) -> None:
    ...
    for col_idx in range(metric_start_col, metric_start_col + len(REQUIRED_EPOCHS) * cols_per_epoch):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("experiments/04_lora_finetuning/report/epoch_ablation.xlsx"),
