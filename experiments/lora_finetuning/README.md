...
This experiment produces six figures, each living with the sub-experiment it belongs to:
...
- [`base_vs_lora/figures/fig_lora_base_vs_lora.pdf`](base_vs_lora/figures/fig_lora_base_vs_lora.pdf) — one chart per model size (3B/7B), each with 5 metric groups and `qwen_base_few_shot`/`qwen_lora_zero_shot` bars. See [`base_vs_lora/README.md`](base_vs_lora/README.md).
...
The first two are copied to [`poster/figures/`](../../poster/figures/) for the poster; `base_vs_lora`'s, `leakage_check`'s, and both `few_shot_ablation` figures are not currently included there.
