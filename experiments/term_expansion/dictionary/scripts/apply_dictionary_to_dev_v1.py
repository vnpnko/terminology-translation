...
Writes ``{lang}_dev_v1_dictionary.jsonl`` to ``shared/data/dev_v1/dev_v1_dictionary/``
only — never overwrites the original inputs. Reads the dictionary from
``experiments/term_expansion/dictionary/data/dev_v2_dictionary/`` and enriches
``proper_terms`` in dev_v1 (``shared/data/dev_v1/dev_v1_original/``) using
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing output in shared/data/dev_v1/dev_v1_dictionary/",
    )
