...
- **`remove_overlap_report.json` and the `dev_v2/deduped/` directory do not exist anywhere in the current `poster-correction` branch.** They only survive in git history (commit `5f9e31f`) and in unrelated old worktrees. The script that would regenerate them, `src/data_preparation/remove_dev_v2_overlap.py`, imports path constants from `term_utils.py` pointing at `data/raw/`, `data/interim/`, `data/processed/` — none of which exist after the "restructure for open-sourcing" (commit `83d1001`); the script cannot run against the repo as checked out today.
  ...
  - **Fix:** either restore `remove_dev_v2_overlap.py` to work against the current `data/dev_v2/` layout and re-run it (regenerating the report + a clearly-named deduped output), or just hardcode the numbers above directly into the paper's data section with a citation to commit `5f9e31f`. The current silent gap — "original" dev_v2 in the tree being the un-deduplicated 2000-line version with no visible trace of the 500-line removal — is exactly the kind of thing that makes the LoRA results look "not honest" if a reader traces the data lineage themselves.
...
- **Few-shot is split across two independently-produced, inconsistently-named result trees**, unified only inside `src/analysis/figure_exp5.py`:
