...
3. **Dead export pipeline**: delete `run_registry.json`'s unused `workbook` fields and the `export_finetuning_report.py`/`sheet_builders.py`/`metrics_parser.py` files outright, or leave them as reference?
