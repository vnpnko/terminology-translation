...
- **Tables:** five comparison workbooks in `experiments/04_lora_finetuning/report/` (see [experiment README § Report tables](../experiments/04_lora_finetuning/README.md#report-tables)): `good vs bad data.xlsx`, `base_few_shots_vs_lora_1_epoch.xlsx`, `best_models.xlsx`, `epoch_ablation.xlsx`, `few_shots_ablation.xlsx`. These replaced the old hand-assembled `comparisons.xlsx`. `good vs bad data.xlsx` is partly script-generated (`fill_good_vs_bad_gpt.py`), and `epoch_ablation.xlsx`/`few_shots_ablation.xlsx` are fully script-generated (`compare_epochs_to_excel.py`/`compare_few_shots_to_excel.py`); the remaining 2 (`base_few_shots_vs_lora_1_epoch.xlsx`, `best_models.xlsx`) are hand-assembled with no regenerating script — `run_registry.json`'s own `workbook` field still points at the never-produced `results_Qwen2.5-{3B,7B}.xlsx`/`results_GPT4o-mini.xlsx` legacy pipeline, which is dead and unused (see ISSUES §2).
...
| `experiments/04_lora_finetuning/report/base_few_shots_vs_lora_1_epoch.xlsx` | hand-assembled (no script) | Base-few-shot vs LoRA-1-epoch table (§3.4) |
