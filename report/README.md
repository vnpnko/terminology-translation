...
- **Tables:** five comparison workbooks in `experiments/04_lora_finetuning/report/` (see [experiment README § Report tables](../experiments/04_lora_finetuning/README.md#report-tables)): `good vs bad data.xlsx`, `base_few_shots_vs_lora_1_epoch.xlsx`, `best_models.xlsx`, `epoch_ablation.xlsx`, `few_shots_ablation.xlsx`. These replaced the old hand-assembled `comparisons.xlsx`. `good vs bad data.xlsx` is partly script-generated (`fill_good_vs_bad_gpt.py`), and `epoch_ablation.xlsx`/`few_shots_ablation.xlsx`/`base_few_shots_vs_lora_1_epoch.xlsx` are fully script-generated (`compare_epochs_to_excel.py`/`compare_few_shots_to_excel.py`/`compare_base_vs_lora_to_excel.py`); the remaining file (`best_models.xlsx`) is hand-assembled with no regenerating script — `run_registry.json`'s own `workbook` field still points at the never-produced `results_Qwen2.5-{3B,7B}.xlsx`/`results_GPT4o-mini.xlsx` legacy pipeline, which is dead and unused (see ISSUES §2).
...
| `experiments/04_lora_finetuning/report/base_few_shots_vs_lora_1_epoch.xlsx` | `experiments/04_lora_finetuning/scripts/compare_base_vs_lora_to_excel.py` | Base-few-shot vs LoRA-1-epoch table (§3.4) |
| `experiments/04_lora_finetuning/report/best_models.xlsx` | hand-assembled (no script) | Best LoRA vs GPT table (§3.4) |
...
| `experiments/04_lora_finetuning/report/few_shots_ablation.xlsx` | `experiments/04_lora_finetuning/scripts/compare_few_shots_to_excel.py` | Few-shot ablation table (§3.4.1) |
| `experiments/04_lora_finetuning/report/good vs bad data.xlsx` | `experiments/04_lora_finetuning/scripts/fill_good_vs_bad_gpt.py` (2 of 5 blocks) + manual edits | Leakage honesty-check table (§3.4.2) |
