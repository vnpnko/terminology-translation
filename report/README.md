...
- **Tables:** five comparison workbooks in `experiments/04_lora_finetuning/report/` (see [experiment README § Report tables](../experiments/04_lora_finetuning/README.md#report-tables)): `leakage_honesty_check.xlsx`, `base_few_shot_vs_lora_zero_shot_1_epoch.xlsx`, `best_models.xlsx`, `epoch_ablation.xlsx`, `zero_shot_vs_few_shot_ablation.xlsx`, all fully script-generated. `run_registry.json`'s own `workbook` field still points at an unused `results_Qwen2.5-{3B,7B}.xlsx`/`results_GPT4o-mini.xlsx` pipeline (see `ISSUES.md`). `best_models.xlsx` cites the **2-epoch** LoRA run as "best" — see the experiment README's note on the overfitting evidence behind that choice.
...
**What was actually done** ([`experiments/04_lora_finetuning/scripts/remove_dev_v2_overlap.py`](../experiments/04_lora_finetuning/scripts/remove_dev_v2_overlap.py) implements this pipeline; it has not been re-run in the current tree, so the numbers below are the pipeline's known result rather than something regeneratable from a live report file — see `ISSUES.md`):
1. dev_v2 starts at **2000 lines/language** (`data/dev_v2/dev_v2_original/*.jsonl`).
...
- `leakage_honesty_check.xlsx` is fully generated by `compare_leakage_honesty_check_to_excel.py` — every cell (GPT, `qwen_base`, `qwen_lora`) is derived directly from `metrics_summary.json`. `qwen_base/test_cleaned_by_sentences/data_good/metrics_summary.json` is a real, verified good-filtered subset.
- Only run for Qwen **7B**, not 3B.
- `data/dev_v2/dev_v2_training/*.jsonl` and `experiments/04_lora_finetuning/data/training/*.jsonl` hold the same derived, post-removal training data checked in twice under two paths, with nothing in the repo indicating which is canonical — pick one before writing up exact training-set sizes.
