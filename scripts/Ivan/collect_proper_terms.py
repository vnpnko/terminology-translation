#!/usr/bin/env python3
"""
Collect ``proper_terms`` pairs from ASAP v1 and v2 dev JSONL files.
...
Usage::
    ...
    python collect_proper_terms.py -t es -o ../../proper\\ terms/asap/enes_pairs.jsonl
...
V1_DIR = REPO_ROOT / "data" / "asap data v1" / "orig data"
V2_DIR = REPO_ROOT / "data" / "asap data v2" / "with terms"
DEFAULT_OUT_DIR = REPO_ROOT / "proper terms" 
...
def default_output(lang: LangConfig) -> Path:
    return DEFAULT_OUT_DIR / f"proper_term_pairs_{lang.pair}.jsonl"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Collect proper_terms EN→target pairs from ASAP v1+v2 into JSONL."
    )
    parser.add_argument(
        "-t",
        "--target-lang",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output JSONL (default: proper terms/asap/proper_term_pairs_<pair>.jsonl)",
    )
    parser.add_argument(
        "--include-count",
        action="store_true",
