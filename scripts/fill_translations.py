...
API key: set OPENROUTER_API_KEY in ``scripts/.env`` (see ``scripts/.env.example``).
...
DEFAULT_INPUT_DIR = ROOT / "new data"
...
def load_dotenv(path: Path = ENV_FILE) -> None:
    """Load KEY=VALUE lines from scripts/.env (does not override existing env vars)."""
    if not path.is_file():
        return
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="OpenRouter API key (default: scripts/.env or OPENROUTER_API_KEY)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    if not args.dry_run and not args.api_key:
        raise SystemExit(
            ...
            "(see scripts/.env.example) or pass --api-key."
        )
